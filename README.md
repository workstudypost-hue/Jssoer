# المنصة التعليمية الإلكترونية — Edu Platform

هذا هو الهيكل الأساسي (Scaffold) للمنصة، مبني وفق التصميم المعماري الكامل المُتفق عليه.
يغطي حاليًا: **الأساس التقني + نظام المصادقة المزدوج + RBAC**، وهو نقطة الانطلاق لبناء بقية الأنظمة (المالي، الفيديو، الذكاء الاصطناعي، التقارير...) تباعًا.

## البنية

```
edu-platform/
├── backend/          # NestJS API
│   ├── prisma/
│   │   ├── schema.prisma   # كل نماذج قاعدة البيانات المصمَّمة
│   │   └── seed.ts         # الأدوار + الصلاحيات + أول super_admin
│   └── src/
│       ├── modules/
│       │   ├── auth/       # staff/ (دعوة) + public/ (تسجيل ذاتي + OTP)
│       │   ├── users/
│       │   ├── rbac/       # فحص الصلاحيات الديناميكي
│       │   └── prisma/
│       └── common/         # Guards + Decorators مشتركة
├── frontend/         # Next.js (App Router) + next-intl (4 لغات + RTL)
├── docs/             # وثائق معمارية إضافية
└── docker-compose.yml
```

## التشغيل المحلي

### 1) قاعدة البيانات و Redis
```bash
docker-compose up -d
```

### 2) Backend
```bash
cd backend
cp .env.example .env     # ثم عدّل القيم الحساسة
npm install
npx prisma migrate dev --name init
npx prisma db seed       # ينشئ الأدوار + super_admin أولي
npm run start:dev
```
سيعمل على: `http://localhost:4000/api/v1`

بيانات دخول super_admin الافتراضية (من الـ seed):
- البريد: `admin@platform.local`
- كلمة المرور: `ChangeMe123!`
⚠️ **غيّرها فورًا** — عرّفها عبر `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD` في `.env` قبل أول seed في بيئة حقيقية.

### 3) Frontend
```bash
cd frontend
npm install
npm run dev
```
سيعمل على: `http://localhost:3000` (يُحوَّل تلقائيًا إلى `/ar`)

## ما هو مُنفَّذ فعليًا الآن

- ✅ Prisma Schema كامل (مستخدمون، أدوار/صلاحيات، دورات، طلبات خاصة، جلسات، **نظام مالي كامل**)
- ✅ مسار مصادقة الموظفين: دعوة → قبول → تسجيل دخول منفصل (`/auth/staff/*`)
- ✅ مسار التسجيل الذاتي: تسجيل → OTP → تفعيل (`/auth/public/*`)
- ✅ موافقة إدارية معلّقة تلقائيًا لكل مدرب جديد (`instructor_profiles.account_status`)
- ✅ RBAC ديناميكي بالكامل من قاعدة البيانات (`PermissionsGuard` + `@RequirePermissions`)
- ✅ سجل نشاط أساسي (`activity_logs`) لعمليات الموظفين الحساسة
- ✅ **نظام الدفع**: Payment Gateway Adapter موحّد (Stripe فعلي عبر SDK، Tabby كهيكل BNPL)
- ✅ **خطط التقسيط**: تجميد سعر الصرف + توليد أقساط تلقائي (`PaymentPlansService`)
- ✅ **مستحقات المدربين**: احتساب صحيح (gross → عمولة بوابة → تقسيم نسبة) + موافقة مزدوجة
  إلزامية فوق 500$ مع منع تطابق المُوافِق الأول/الثاني برمجيًا
- ✅ **مسار الطلبات الخاصة**: State Machine كاملة (26 قاعدة انتقال) من الإنشاء حتى التسليم/النزاع
- ✅ **حماية الفيديو (جديد)**: `VideoSecurityService` — جلسات تشغيل بـ Signed Token قصير العمر
  (5 دقائق) + Heartbeat للتجديد، فحص `enrollment.access_status='full'` قبل السماح بالتشغيل
  (يتكامل مع سياسة التعثر 7 أيام)، تسجيل أحداث أمنية (DevTools، محاولات تنزيل...) وتصعيد
  تلقائي (بدون إجراء على الحساب) عند تجاوز عتبات: 3+ IPs مختلفة خلال 24 ساعة، أو 5+ محاولات
  DevTools خلال أسبوع
- ✅ **لوحة مراجعة الأمان الموحّدة (جديد)**: `SecurityReviewService` تخدم كل أنواع الاشتباه
  (فيديو + تواصل) بنفس الآلية - تنبيه بشري فقط، القرار الحساس (تعليق حساب) يبقى توثيقًا/توصية
  فقط هنا وليس تنفيذًا تلقائيًا
- ✅ **الرقابة على التواصل (جديد)**: `MessagesService` بخط دفاع Layer 1 (Regex) كامل - حظر فوري
  لثقة عالية (رقم/بريد صريح)، إخفاء جزئي + تصعيد لثقة متوسطة، مرور مع تسجيل صامت لثقة منخفضة؛
  تصعيد تراكمي (Progressive Enforcement) عند 5+ مخالفات خلال 30 يومًا؛ استثناء لقناة الدعم
  الرسمية (`support_ticket` + موظف موثَّق)
- ✅ **الذكاء الاصطناعي (جديد)**: `AiGatewayService` كنقطة دخول مركزية وحيدة لأي استدعاء
  Claude API - يفرض حدود استخدام مرنة بالكامل من DB (`AiUsageGuardService`، لا أرقام ثابتة
  بالكود) وتوجيهًا متعدد النماذج يُقرأ من DB (`AiModelRouterService` - Haiku/Sonnet/Opus حسب
  الميزة + fallback تلقائي)؛ `CourseAssistantService` لمساعد الطالب (RAG محصور بالدورة -
  Stub جاهز البنية) وتوليد بنك أسئلة للمدرب (يدخل `draft` إلزاميًا حتى موافقته الصريحة عبر
  `QuestionBankReviewService`)
- ✅ **التقارير (جديد)**: `ReportsService` بنمط الاستعلامات الآمنة (Whitelisted Queries عبر
  `REPORT_QUERIES` registry - ممنوع أي SQL حر)، النطاق الشخصي (`requesterId`) يُحقن دائمًا
  من الـ JWT وليس من مدخلات الطلب، 7 تقارير أساسية جاهزة (أكاديمي/إداري/مالي) بصلاحيات RBAC
  مطابقة تمامًا لمصفوفة الأدوار المصمَّمة
- ✅ Frontend: توجيه i18n لـ 4 لغات مع RTL تلقائي للعربية
- ✅ **Cron Jobs فعلية (جديد)**: `SchedulerModule` عبر BullMQ - تحصيل الأقساط اليومي (02:00)
  مع حجب جزئي فعلي عند التعثر بعد 7 أيام، تحديث أسعار الصرف كل 6 ساعات، توليد مستحقات
  المدربين اليومي (03:00) — كلها Jobs متكررة حقيقية مربوطة فعليًا، وليست دوالًا معزولة
  كانت موجودة في الكود دون أي استدعاء (كما اكتُشف عند التدقيق)

## 🎉 حالة الإنجاز: الأنظمة الثمانية المصمَّمة عبر جلسة التصميم الكاملة ممثَّلة جميعها في الكود
(مصادقة مزدوجة، RBAC، نظام مالي، طلبات خاصة، حماية فيديو، رقابة تواصل، ذكاء اصطناعي، تقارير)

## تشغيل الـ Cron Jobs محليًا

الـ Jobs تُسجَّل تلقائيًا عند إقلاع الخادم (`SchedulerService.onModuleInit`)، لكن **تتطلب Redis
يعمل فعليًا** (`docker-compose up -d`) وإلا فشل الاتصال عند الإقلاع.

## الخطوات التالية (راجع `docs/production-notes.md`)

كل ميزة مؤجَّلة عمدًا لهذه المرحلة موسومة بـ `TODO(production)` في الكود، وموثّقة بالتفصيل في `docs/production-notes.md` — بما فيها: تكامل OTP الفعلي (Twilio)، OAuth (Google/Microsoft)، الأنظمة المالية، حماية الفيديو، الذكاء الاصطناعي، والتقارير.

## المتابعة عبر Claude Code

هذا المشروع مصمَّم للاستكمال عبر **Claude Code**. بعد فك الضغط، افتح المجلد وابدأ الجلسة بطلب مثل:
> "اقرأ docs/production-notes.md وابدأ بتنفيذ نظام الدفع والأقساط كما هو موثّق في نفس الملف"
